...
* **PHOENIX\_ALLOWED\_ORIGINS:** A comma-separated list of browser origins allowed to call Phoenix from a different origin. Set this when your Phoenix UI or other browser clients are served from a different scheme, host, or port. Leave it unset if all browser traffic is same-origin.

* **PHOENIX\_COOKIES\_PATH:** The URL path where Phoenix auth cookies are valid. Defaults to `/`. Set this to your Phoenix subpath when serving Phoenix behind a reverse proxy such as `/phoenix`.
...
### Browser Access and Auth Cookies

Most self-hosted deployments work without extra browser settings when the Phoenix UI and API are served from the same origin. You only need the settings below when Phoenix is behind a subpath or when browser traffic comes from a different same-site origin.

* Use `PHOENIX_ALLOWED_ORIGINS` to allow specific same-site browser origins to call Phoenix across origins.
* Use `PHOENIX_CSRF_TRUSTED_ORIGINS` for login, OAuth, and other browser flows that send `origin` or `referer` headers.
* Use `PHOENIX_COOKIES_PATH` to scope auth cookies to the Phoenix path when serving the app behind a reverse proxy subpath.

<Note>
If Phoenix is served at a subpath such as `/phoenix`, set both `PHOENIX_HOST_ROOT_PATH=/phoenix` and `PHOENIX_COOKIES_PATH=/phoenix` so routes and login cookies stay aligned.
</Note>
