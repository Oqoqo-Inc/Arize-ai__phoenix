...
* **PHOENIX\_ALLOWED\_ORIGINS:** A comma-separated list of origins permitted for cross-origin requests (CORS). When unset (the default), CORS is disabled and cross-origin browser requests will be rejected. Set this when Phoenix is accessed from a different origin than the one it is served from — for example, when embedding the UI in another application or calling the API from a browser on a different domain. Avoid using overly broad values (e.g., `*`) in production as this weakens origin security.
