...
* **PHOENIX\_MAX\_SPANS\_QUEUE\_SIZE:** Maximum number of spans Phoenix will hold in memory before temporarily rejecting new trace uploads. Defaults to `20000`. Increase this only if the server has enough memory and the database can keep up with ingestion.
...
Phoenix buffers spans before writing them to the database. `PHOENIX_MAX_SPANS_QUEUE_SIZE` controls how many spans can wait in that queue.

<Info>
If the queue is full, Phoenix temporarily rejects new trace uploads with HTTP `503 Service Unavailable` on the HTTP collector and `RESOURCE_EXHAUSTED` on the gRPC collector. This usually means the server does not have enough headroom to absorb the current ingestion rate.
</Info>

As a rough guide, the default queue size of `20000` spans can use about 1 GiB of memory when spans average around 50 KiB each. Increase the limit only if the server has enough memory and the database can keep up.
