...
* **PHOENIX\_COOKIES\_PATH:** The `Path` attribute used for Phoenix authentication cookies. Defaults to `/`. When Phoenix is hosted under a subpath (for example, behind a reverse proxy with `PHOENIX_HOST_ROOT_PATH=/phoenix`), set this to the same subpath (for example, `/phoenix`) so that login cookies are sent with application requests. If this is left at `/` or set incorrectly in a subpath deployment, users may see repeated logins or redirect loops because cookies are not attached to their requests.
...
* **PHOENIX\_ALLOWED\_ORIGINS:** A comma-separated list of allowed CORS origins for browser and API requests to Phoenix. In cross-origin setups, set this to the exact origins of your UI or reverse proxy (for example, `https://app.example.com`). Leaving this unset or empty means only same-origin requests are allowed; setting it too broadly (such as `*`) weakens origin isolation and is not recommended for production. This setting controls CORS behavior and is separate from **PHOENIX\_CSRF\_TRUSTED\_ORIGINS**, which governs CSRF protection.
