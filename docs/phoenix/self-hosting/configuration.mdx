...
* **PHOENIX\_COOKIES\_PATH:** The path scope for authentication cookies. Defaults to `/`. When Phoenix is served under a sub-path behind a reverse proxy (for example, `/phoenix`), set this to the same sub-path so that browser requests include the auth cookies for Phoenix endpoints.
...
* **PHOENIX\_ALLOWED\_ORIGINS:** A comma-separated list of allowed CORS origins for cross-origin requests to the Phoenix API. When unset, only same-origin browser requests are permitted. Set this when accessing Phoenix from a different origin (for example, a separate frontend or management UI). Be cautious when broadening this list, as overly permissive values reduce origin isolation.
