...
* **PHOENIX\_MIGRATE\_INDEX\_CONCURRENTLY:** When set to `True`, Phoenix uses concurrent index creation during PostgreSQL migrations. This is helpful for rolling deployments where an older Phoenix instance may still be writing traces while the new instance starts up. It is slower than the default migration path and is ignored for SQLite.
...
For PostgreSQL rolling deployments, set `PHOENIX_MIGRATE_INDEX_CONCURRENTLY=true` on the new Phoenix instance so index creation does not block writes from the older instance during startup.

<Warning>
Concurrent index creation takes longer than the default migration path and Phoenix still waits for the migration to finish before the new instance is ready. This setting is ignored for SQLite.
</Warning>

See [Migrations](/docs/phoenix/self-hosting/upgrade/migrations) for upgrade guidance.
