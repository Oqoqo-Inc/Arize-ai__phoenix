...
* **PHOENIX\_COOKIES\_PATH:** The path scope applied to all Phoenix authentication cookies. Defaults to `/`, which scopes cookies to the entire host. In reverse-proxy or subpath deployments (for example, when Phoenix is served at `/phoenix` behind a gateway), set this to that subpath (for example, `/phoenix`) so login and refresh cookies are included on browser requests to the UI and APIs.
...
* **PHOENIX\_ALLOWED\_ORIGINS:** A comma-separated list of origins that are allowed to make cross-origin requests to the Phoenix HTTP API and UI. When unset, CORS is disabled and only same-origin browser requests will succeed. Each origin should include scheme, host, and port (for example, `https://app.example.com,https://admin.example.com`). Avoid using overly broad patterns such as `*`, especially with credentialed requests, as they weaken origin security and may be rejected by browsers.
