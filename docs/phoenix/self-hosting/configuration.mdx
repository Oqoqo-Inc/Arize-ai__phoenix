...
* **PHOENIX\_ALLOWED\_ORIGINS:** A comma-separated list of origins that are allowed to make cross-origin browser requests to the Phoenix server (CORS). If this variable is not set or is empty, cross-origin requests are not allowed and CORS is effectively disabled. When set, only the listed origins are permitted. Values should include scheme and host (for example, `https://phoenix.example.com`). Defaults to `None` (CORS disabled).

* **PHOENIX\_COOKIES\_PATH:** The path scope for authentication cookies issued by Phoenix. Defaults to `/`, which scopes cookies to the entire origin. When running Phoenix behind a reverse proxy or under a subpath (for example, `/phoenix`), set this to the mount path so that login and authenticated requests continue to work correctly.
