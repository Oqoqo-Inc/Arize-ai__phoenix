...
### Cross-origin and cookie scope settings

When authentication is enabled and Phoenix is accessed from other browser-based applications or through a reverse proxy, you may also need to configure:

- <strong>PHOENIX_ALLOWED_ORIGINS</strong> – A comma-separated list of browser origins that are allowed to make cross-origin requests to Phoenix. This is required when your Phoenix UI or REST APIs are loaded from a different origin (for example, an internal portal or a separate frontend domain). If the origin is not listed, browsers will block the requests, which can surface as failed logins or broken UI/API calls.
- <strong>PHOENIX_COOKIES_PATH</strong> – The path attribute for Phoenix authentication cookies. In subpath or reverse-proxy deployments (for example, serving Phoenix at <code>/phoenix</code>), setting this path so it matches the served path ensures that login cookies are sent on subsequent requests. If it is misconfigured, users may be redirected back to the login page because their browser is not sending the auth cookie.

See the <a href="../configuration#environment-variables">Environment Variables</a> reference for full details and defaults.
