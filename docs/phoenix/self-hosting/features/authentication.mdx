...
When Phoenix is accessed from a different origin than where the server is hosted (for example, behind a separate reverse proxy or UI domain), use <code>PHOENIX_ALLOWED_ORIGINS</code> to configure which browser origins are allowed to make cross-origin requests to Phoenix. Set this to the exact origins of your UI or proxy and avoid overly broad values like <code>*</code> in production. See the <strong>Server Configuration</strong> section of the self-hosting configuration page for full details.

If Phoenix is served under a subpath (for example, with <code>PHOENIX_HOST_ROOT_PATH=/phoenix</code> behind a reverse proxy), set <code>PHOENIX_COOKIES_PATH</code> to the same subpath (for example, <code>/phoenix</code>) so that authentication cookies are sent with application requests. If this is left at <code>/</code> or set incorrectly, users may experience repeated login prompts or redirect loops because cookies are not attached to subpath requests.
