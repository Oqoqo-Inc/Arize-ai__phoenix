...
<Columns cols={2}>
  <Card title="TypeScript Tutorial" icon="js" href="https://github.com/Arize-ai/phoenix/tree/main/js/examples/apps/tracing-tutorial">
...
<Info>
  See the full session-enabled agent code [here](https://github.com/Arize-ai/phoenix/tree/main/js/examples/apps/tracing-tutorial/support-agent.ts).
</Info>
...
<Columns cols={2}>
  <Card title="TypeScript Tutorial" icon="js" href="https://github.com/Arize-ai/phoenix/tree/main/js/examples/apps/tracing-tutorial/support-agent.ts">
    Run the sessions demo: `pnpm sessions`
  </Card>

  <Card title="Python Tutorial" icon="python" href="https://colab.research.google.com/github/Arize-ai/phoenix/blob/main/tutorials/tracing/python_tracing_quickstart.ipynb" description="End-to-end Python tutorial">
  </Card>
</Columns>
...
<Info>
  See the session evaluation code [here](https://github.com/Arize-ai/phoenix/tree/main/js/examples/apps/tracing-tutorial/evaluate-traces.ts).
</Info>
...
<Columns cols={2}>
  <Card title="TypeScript Tutorial" icon="js" href="https://github.com/Arize-ai/phoenix/tree/main/js/examples/apps/tracing-tutorial/evaluate-traces.ts">
...
# Programmatic Session APIs

Sessions are first-class resources in Phoenix. In addition to the Sessions UI, you can use the Python and TypeScript clients to:

- Reconstruct the ordered turns in a session
- Delete sessions (individually or in bulk)
- Log and update session-level annotations

These APIs build on the semantic conventions you used earlier in this tutorial (`input.value`, `output.value`, and `session.id`).

## Reconstructing Session Turns

Use the session turns API to rebuild a conversation transcript directly from Phoenix, instead of relying on your application logs.
...
- Turns are ordered chronologically by the underlying trace start time.
- Phoenix uses the root span's `input.value` and `output.value` attributes to reconstruct each turn.
- If those attributes are missing, the `input`/`output` fields may be empty for that turn.

<Info>
The session turns API is experimental and may change in future releases. See the Session Turns API release note for version requirements.
</Info>

## Deleting Sessions

When you run experiments or collect test data, you may want to remove sessions and their traces after you're done. Phoenix exposes deletion APIs in both clients.
...
client.sessions.delete(session_id="my-session-id")

# Bulk delete multiple sessions
client.sessions.bulk_delete(session_ids=["session-1", "session-2", "session-3"])
```
...
<Warning>
Deleting a session is a cascading operation. When a session is deleted, all associated traces, spans, and session annotations are permanently removed. Use bulk delete carefully in production environments.
</Warning>

Behavior to keep in mind:

- All identifiers in a bulk delete call must be the same type (either all Phoenix GlobalIDs or all user-provided session IDs).
- Non-existent IDs are safely ignored in bulk deletes.
- Session deletion requires recent Phoenix server versions; see the release notes for exact version numbers.

## Logging Session Annotations Programmatically

Earlier in this tutorial you logged session-level annotations from an evaluation job. You can also log or update annotations directly using the client APIs.
...
await logSessionAnnotations({
  sessionAnnotations: [
    {
      ...
      score: 0.95,
      annotatorKind: "LLM",
      metadata: { model: "gpt-5", turnCount: 6 },
    },
    {
      sessionId: "my-session-id",
      name: "resolution_status",
  ...
  // Set sync to true to wait for IDs
  sync: false,
});
```
...
session_annotations = [
    SessionAnnotationData(
        ...
        name="conversation_coherence",
        annotator_kind="LLM",
        result={"label": "coherent", "score": 0.95},
        metadata={"model": "gpt-5", "turnCount": 6},
    ),
    SessionAnnotationData(
        session_id="my-session-id",
        name="resolution_status",
        annotator_kind="LLM",
        result={"label": "resolved", "score": 1.0},
        metadata={"model": "gpt-5", "turnCount": 6},
    ),
]
...
- An annotation must include at least a label or a score to be useful.
- Use the `metadata` field to capture context like model name, dataset slice, or experiment ID.
- Use a stable `identifier` when you want to update an existing annotation instead of creating a new one.

For more details on the underlying REST endpoints and full parameter reference, see:

- [Sessions REST API overview](/docs/phoenix/sdk-api-reference/rest-api/api-reference/sessions)
- [Session Turns API release notes](/docs/phoenix/release-notes/03-2026/03-11-2026-session-turns-api)
