...
After you attach session IDs, you can manage the resulting session records through the SDKs:
...
<Warning>
Deleting a session removes its traces, spans, and annotations. Bulk deletion requires identifiers of the same type, and non-existent IDs are skipped.
</Warning>

<Note>
`get_session_turns()` and `getSessionTurns()` reconstruct turns from each trace's root span. If the root span does not record `input.value` and `output.value`, the turn list still returns, but those fields may be empty.
</Note>
