...
## Reconstruct conversations programmatically

Once you are logging traces with a session identifier, you can reconstruct the ordered turns of a conversation directly from Phoenix.
...
<Note>
The Session Turns API is experimental and requires recent Phoenix versions. See the Session Turns release notes for version requirements and additional details.
</Note>

## Managing and deleting sessions

You can also manage the lifecycle of sessions from code for cleanup and privacy workflows.
...
<Tabs>
  ...
  # Delete a single session and its associated traces/spans/annotations
...
<Warning>
Deleting a session cascades to its related traces, spans, and annotations. Use bulk deletion carefully, especially in production projects. See the Sessions REST API reference for endpoint details and server version requirements.
</Warning>
