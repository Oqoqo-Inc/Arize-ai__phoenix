...
## Querying Session Turns

You can programmatically retrieve the ordered input/output turns for any session using the Phoenix client. Each turn corresponds to one trace in the session and exposes the root span's `input.value` and `output.value` attributes.

<Note>
`get_session_turns` / `getSessionTurns` requires Phoenix server >= 13.5.0. If the root span does not carry `input.value` / `output.value` attributes (set via OpenInference semantic conventions), the `input` and `output` fields will be absent from each turn.
</Note>
...
<Tabs>
  <Tab title="Python" icon="python">
    ...
    for turn in turns:
        print(f"[{turn['start_time']}]")
        print(f"  Input:  {turn.get('input', {}).get('value', '<no input>')}")
        print(f"  Output: {turn.get('output', {}).get('value', '<no output>')}")
    ```

    Each `SessionTurn` dict contains:
    - `trace_id` — the trace ID for this turn
    - `start_time` / `end_time` — ISO-8601 timestamps
    - `input` / `output` — optional dicts with `value` (str) and `mime_type` (str, optional)
    - `root_span` — the full root span object, if found
  </Tab>
  ...
  <Tab title="TypeScript" icon="js">
    ...
    - `traceId` — the trace ID for this turn
    - `startTime` / `endTime` — ISO-8601 timestamps
    - `input` / `output` — optional objects with `value` (string) and `mimeType` (string, optional)
    - `rootSpan` — the full root span, if found

    <Note>
    `getSessionTurns` is currently experimental and its interface may change in future releases.
    </Note>
  </Tab>
</Tabs>
...
Use the Phoenix client to delete one or many sessions. Deletion is permanent and cascades to all associated traces, spans, and annotations.

<Warning>
Session deletion is irreversible. All traces, spans, and annotations belonging to the deleted session(s) are permanently removed. Requires Phoenix server >= 13.13.0.
</Warning>
...
<Tabs>
  <Tab title="Python" icon="python">
    ...
    **Notes:**
    - `bulk_delete` raises `ValueError` if `session_ids` is empty.
    - All identifiers must be the same type — either all user-provided session ID strings or all Phoenix GlobalIDs (base64-encoded). Mixing types is not supported.
    - Non-existent session IDs are silently skipped.
  </Tab>
  ...
  <Tab title="TypeScript" icon="js">
    ...
    **Notes:**
    - `deleteSession` throws if the session is not found.
    - `deleteSessions` silently skips non-existent IDs.
    - All identifiers must be the same type — either all user-provided strings or all Phoenix GlobalIDs. Mixing types is not supported.
  </Tab>
</Tabs>

## Annotating Sessions

You can attach scores, labels, and metadata to entire sessions using session annotations. This is useful for rating conversation quality, tracking outcomes, or running LLM-as-judge evaluations at the session level.

<Note>
Session annotations require Phoenix server >= 12.0.0.
</Note>
...
<Tabs>
  <Tab title="TypeScript" icon="js">
    ...
    const results = await logSessionAnnotations({
      ...
      sync: false,  // set to true to wait for annotation IDs in the response
    });
    ```
  </Tab>
</Tabs>
