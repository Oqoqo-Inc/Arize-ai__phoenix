...
Once you are attaching a `session_id` to your traces, you can list and fetch sessions from your application code.
...
for session in sessions:
    print(session["session_id"], len(session["traces"]))
...
for (const session of sessions) {
  console.log(session.sessionId, session.traces.length);
}
...
## Reconstruct conversation turns for a session

Use the session turns API to reconstruct ordered input/output pairs for a conversation.
...
<Note>
The session turns API reconstructs turns from traces instrumented with OpenInference semantic attributes and is experimental. The shape of `SessionTurn` may change in future versions.
</Note>
...
Delete individual sessions or multiple sessions when you need to clean up test data, enforce retention policies, or remove conversations.
...
<Warning>
Deleting a session permanently removes the session and its associated traces, spans, and annotations. This operation cannot be undone.
</Warning>
...
Session-level annotations let you attach quality scores, labels, and notes to entire conversations.
