...
## Next steps: querying and managing sessions

After you've instrumented your application and started generating sessions, you can use the Phoenix client SDKs to reconstruct conversations and manage session lifecycle.
...
for turn in turns:
    print(turn["input"]["value"], "->", turn["output"]["value"])
```
...
For more on reconstructing conversations, deleting sessions, and working with session-level annotations, see the main [Sessions](/docs/phoenix/tracing/llm-traces/sessions) page and the Python and TypeScript SDK references.
