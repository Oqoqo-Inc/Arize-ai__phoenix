...
Once you are logging sessions, you can also query them programmatically. Phoenix exposes APIs to list sessions, reconstruct ordered input/output turns from traces, and export sessions for offline analysis.

<Tabs>

  <Tab title="Python" icon="python">

  Use the Python client to fetch the ordered turns for a session:
  ...
  Use the TypeScript client to fetch session turns:
  ...
  for (const turn of turns) {
    console.log(`${turn.input?.value} -> ${turn.output?.value}`);
  }
  ```
...
## Deleting Sessions Programmatically

You can delete sessions through the client SDKs. Deleting a session cascades to its associated traces, spans, and annotations.
...
<Warning>
Deleting sessions is irreversible and will remove associated traces, spans, and annotations. Test your deletion workflows in non-production environments before applying them to production projects.
</Warning>
