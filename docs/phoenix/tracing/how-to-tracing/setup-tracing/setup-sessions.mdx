...
## Programmatically querying sessions

Once you are attaching a `session_id` to your traces, you can also work with sessions directly from the Phoenix client SDKs.

The following features require recent versions of Phoenix:

- **Session turns API** (reconstruct an ordered list of input/output turns)
  - Python: `arize-phoenix-client` 2.0.0+
  - TypeScript: `@arizeai/phoenix-client` 6.4.0+
- **Session management APIs** (get, list, delete, bulk delete)
  - Server: `arize-phoenix` 13.13.0+ for delete/bulk delete, 13.5.0+ for listing
  - Python: `arize-phoenix-client` 1.31.0+
  - TypeScript: `@arizeai/phoenix-client` 6.1.0+

### Reconstruct a conversation with session turns

Use the Session Turns API to rebuild the ordered conversation for a given session. This API is **experimental** and may change in future releases.
...
<Tabs>
  ...
  # List recent sessions in a project
...
You can list, inspect, and delete sessions (including cascading cleanup of associated traces, spans, and annotations) via the client SDKs.
...
> For more details on the underlying REST endpoints and type definitions, see the Session Turns and Session Management sections in the Phoenix release notes.
