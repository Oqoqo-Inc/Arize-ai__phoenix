...
Once session IDs are attached to your spans and visible in the Sessions tab, you can:

- Reconstruct ordered conversation turns for a session
- Clean up test or expired sessions (including their traces and annotations)
- Log session-level annotations and evaluator results

See the [Sessions tutorial](/docs/phoenix/tracing/tutorial/sessions#programmatic-session-apis) for end-to-end Python and TypeScript examples.
