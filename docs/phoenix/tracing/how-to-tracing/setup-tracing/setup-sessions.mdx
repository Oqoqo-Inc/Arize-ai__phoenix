...
## Querying and managing sessions with the SDKs

Once sessions are configured and visible in the UI, you can use the SDKs to reconstruct conversations and clean up sessions programmatically.

### Reconstruct a conversation from a session ID
...
<Tabs>
  ...
  for idx, turn in enumerate(turns, start=1):
      ...
      print(f"Turn {idx}: {input_val} -> {output_val}")
  ```

  The `get_session_turns` API reconstructs the ordered turns in a session based on trace `start_time`. This API is experimental and requires a recent Phoenix server (13.5.0 or later). Input and output values are populated when your tracing instrumentation records them on the root span.
...
<Warning>
Session deletes are **permanent** and cascade to traces, spans, and annotations for that session. Non-existent session IDs are ignored, but an empty list passed to `client.sessions.bulk_delete` in Python will raise a `ValueError`.

These delete APIs require Phoenix server 13.13.0 or later.
</Warning>
