...
## Query, reconstruct, and clean up sessions

Once you are logging spans with a `session_id`, you can use the Phoenix SDKs to reconstruct a transcript for a session, list sessions, and clean them up.
...
<Tabs>
  ...
  # Get turns for a session as ordered input/output pairs
  ...
  for turn in turns:
      ...
      print(f"Turn {turn['trace_id']}: {input_val} -> {output_val}")
  ...
  // Log annotations at the session level (for evaluations or labels)
...
<Warning>
Session deletion is **destructive** and cascades to associated traces, spans, and annotations. Bulk deletion requires Phoenix server version 13.13.0+ and compatible client versions (see SDK reference pages for version details).
