...
<Tabs>
  <Tab title="Python" icon="python">
    ...
    <Accordion title="Configuration options">
      ...
      | `protocol` | Transport protocol: `"grpc"` or `"http/protobuf"`. If omitted, inferred from `endpoint`: **no path + gRPC port** (default `4317`, configurable by `PHOENIX_GRPC_PORT`) uses gRPC; **`/v1/traces`** uses HTTP/protobuf; otherwise set `protocol` explicitly. |
  ...
  <Tab title="TypeScript" icon="js">
    ...
    <Accordion title="Configuration options">
      ...
      | `batch` | `boolean` | `false` | Use batch span processing |
