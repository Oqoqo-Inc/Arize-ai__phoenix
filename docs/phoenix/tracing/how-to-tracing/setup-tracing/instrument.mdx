...
## 3. Using the @observe decorator (TypeScript)

If you're using TypeScript classes, the `@observe` decorator from `@arizeai/openinference-core` provides a concise way to trace class methods. It automatically captures inputs, outputs, and errors as span attributes.
...
<Info>
`@observe` requires TypeScript with decorator support enabled (`experimentalDecorators` in `tsconfig.json`). For standalone functions, use the wrapper helpers above instead.
</Info>
