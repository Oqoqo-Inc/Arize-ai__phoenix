...
The Phoenix client exposes APIs for working with sessions beyond the UI:

- **Reconstruct conversation turns** — retrieve ordered input/output pairs for each trace in a session (`get_session_turns` / `getSessionTurns`)
- **Delete sessions** — permanently remove a session and all its traces, spans, and annotations (`delete` / `deleteSession`, `bulk_delete` / `deleteSessions`)
- **Annotate sessions** — attach scores, labels, and metadata to entire sessions for evaluation workflows (`logSessionAnnotations`)

See [Setup Sessions](/docs/phoenix/tracing/how-to-tracing/setup-tracing/setup-sessions) for full examples.
