...
* Reconstruct ordered turns from a session's traces for downstream analysis
* Delete one session or bulk-delete sessions when you need to clean up test or stale data
* Track token usage and latency per conversation

<Note>
  The management APIs live on the client SDKs, not just the UI. Use them when you need to export transcripts, automate cleanup, or build session-level workflows outside Phoenix.
</Note>
...
<CardGroup cols={3}>
  ...
  <Card title="Manage Sessions" icon="list" href="/docs/phoenix/sdk-api-reference/rest-api/api-reference/sessions">
    Query, reconstruct, and delete sessions through the API
  </Card>
