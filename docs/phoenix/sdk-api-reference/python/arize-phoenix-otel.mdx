...
<Info>
If `protocol` is not set, Phoenix infers it from `endpoint`:
- gRPC: endpoint has no path and uses the configured gRPC port (default `4317`)
- HTTP/protobuf: endpoint path is exactly `/v1/traces`
- Otherwise, set `protocol` explicitly
</Info>
