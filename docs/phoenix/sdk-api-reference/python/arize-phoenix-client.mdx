...
Retrieve and manage multi-turn conversations. Sessions group related traces from aconversation into a single timeline.
...
```python
# Reconstruct the ordered turns for a session
...
turns = await async_client.sessions.get_session_turns(session_id="my-session")
await async_client.sessions.delete(session_id="my-session")
await async_client.sessions.bulk_delete(session_ids=["session-1", "session-2"])
```

<Warning>
Deleting sessions is irreversible and cascades to all associated traces, spans, and annotations. Use bulk deletion carefully in production environments.
</Warning>

### Reference

- `get(session_id: str, timeout: Optional[int] = 5) -> dict` — retrieve a single session
- `list(project_name: str, limit: Optional[int] = None, timeout: Optional[int] = 5)` — list sessions for a project
- `get_sessions_dataframe(project_name: str, timeout: Optional[int] = 5)` — retrieve sessions as a pandas DataFrame
- `get_session_turns(session_id: str, timeout: Optional[int] = 5)` — reconstruct the ordered turns for a session
- `delete(session_id: str, timeout: Optional[int] = 5) -> None` — delete a single session
- `bulk_delete(session_ids: List[str], timeout: Optional[int] = 5) -> None` — delete multiple sessions at once
