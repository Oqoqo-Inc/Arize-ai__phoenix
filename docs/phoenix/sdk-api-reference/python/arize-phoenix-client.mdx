...
The `get_session_turns` method reconstructs ordered conversation turns for a session by reading `input.value` and `output.value` from the root span of each trace. Each turn includes the trace identifier, input and output payloads, and timing information.

> Note: `get_session_turns` is experimental and available in `arize-phoenix-client` 2.0.0+.
...
When performing bulk deletes, all identifiers in the list must be of the same type (either user-provided session IDs or Phoenix GlobalIDs). Non-existent IDs are skipped.
