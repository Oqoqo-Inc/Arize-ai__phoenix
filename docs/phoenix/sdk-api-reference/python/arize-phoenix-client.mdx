...
Session retrieval methods such as `get()` and `list()` return `v1.SessionData` objects. The session turns API returns `SessionTurn` objects with `input` and `output` values stored as `SessionTurnIO` objects.
...
# Delete one session or several sessions
client.sessions.delete(session_id="test-session")
client.sessions.bulk_delete(session_ids=["test-session-1", "test-session-2"])

# Reconstruct ordered turns for a session
turns = client.sessions.get_session_turns(session_id="my-session-id")
for turn in turns:
    print(turn.trace_id, turn.input.value if turn.input else None)
```

<Warning>
`client.sessions.get_session_turns()` is experimental and may change in future releases.

</Warning>

<Note>
Deleting a session cascades through its traces, spans, and session annotations.
...
await async_client.sessions.delete(session_id="test-session")
await async_client.sessions.bulk_delete(session_ids=["test-session-1", "test-session-2"])
turns = await async_client.sessions.get_session_turns(session_id="my-session-id")
```
