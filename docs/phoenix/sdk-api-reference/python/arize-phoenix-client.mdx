...
# Reconstruct ordered turns for a session (experimental)
...
client.sessions.delete(session_id="old-session-id")
client.sessions.bulk_delete(session_ids=["session-1", "session-2"])
```

`get_session_turns` is **experimental** and requires a recent Phoenix server (13.5.0 or later). It returns turns ordered by `start_time` and only includes input and output values when your tracing instrumentation records them on the root span.

Session delete and bulk delete are **hard** deletes that cascade to traces, spans, and annotations for the deleted sessions. These APIs require Phoenix server 13.13.0 or later.
...
await async_client.sessions.delete(session_id="old-session-id")
