...
for turn in turns:
    print(turn["trace_id"], turn.get("input", {}).get("value"), turn.get("output", {}).get("value"))
```
...
await async_client.sessions.delete(session_id="my-session-id")
df = await async_client.sessions.get_sessions_dataframe(project_name="my-chatbot")
```

<Warning>
Deleting a session removes its traces, spans, and annotations. Bulk deletion requires identifiers of the same type.
</Warning>

<Note>
`get_session_turns()` is experimental and reconstructs turns from each trace's root span. If those spans do not record `input.value` and `output.value`, the turn list still returns, but those fields may be empty.
</Note>
