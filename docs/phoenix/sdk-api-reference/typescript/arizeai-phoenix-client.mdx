...
for (const turn of turns) {
  console.log(turn.traceId, turn.input?.value, turn.output?.value);
}
```
...
<Note>
`getSessionTurns()` is experimental and reconstructs turns from each trace's root span. If those spans do not record `input.value` and `output.value`, the turn list still returns, but those fields may be empty.
</Note>
