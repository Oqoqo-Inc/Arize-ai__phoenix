...
for (const turn of turns) {
  const input = turn.input?.value ?? "<no input>";
  const output = turn.output?.value ?? "<no output>";
  console.log(`[${turn.startTime}] ${input} -> ${output}`);
}
```
...
- `getSession(options: { sessionId: string; client?: PhoenixClient })` — retrieve a single session
- `listSessions(options: { projectName?: string; projectId?: string; project?: string; client?: PhoenixClient })` — list sessions for a project
- `getSessionTurns(options: { sessionId: string; client?: PhoenixClient })` — reconstruct the ordered turns for a session
- `deleteSession(options: { sessionId: string; client?: PhoenixClient })` — delete a single session
- `deleteSessions(options: { sessionIds: string[]; client?: PhoenixClient })` — delete multiple sessions at once
