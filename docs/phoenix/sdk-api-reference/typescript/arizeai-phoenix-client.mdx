...
```javascript
import { deleteSessions } from "@arizeai/phoenix-client/sessions";

await deleteSessions({ sessionIds: ["test-session-1", "test-session-2"] });
```

All session IDs passed to `deleteSessions()` must use the same identifier type.
...
for (const turn of turns) {
  console.log(turn.traceId, turn.input?.value, turn.output?.value, turn.input?.mimeType);
}
```

<Warning>
`getSessionTurns()` is experimental and may change in future releases.

</Warning>

Each turn is returned in chronological order and includes `input` and `output` payloads as `SessionTurnIO` objects.
...
Use `sync: true` when you need annotation IDs returned immediately.
