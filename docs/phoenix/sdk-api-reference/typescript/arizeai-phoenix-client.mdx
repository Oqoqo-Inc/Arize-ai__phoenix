...
The `getSessionTurns` function reconstructs ordered conversation turns for a session by reading the input and output values from the root span of each trace.
...
> Note: session management (get, list, delete, bulk delete) is available in `@arizeai/phoenix-client` 6.1.0+; the Session Turns API is available in 6.4.0+.
