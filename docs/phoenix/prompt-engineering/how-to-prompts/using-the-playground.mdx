...
<Warning>
When **Anthropic Extended Thinking** is enabled, Playground does not send `temperature`, `top_p`, or forced tool choice settings in the request.
These controls may still appear in the UI, but they are ignored for that run.
</Warning>
